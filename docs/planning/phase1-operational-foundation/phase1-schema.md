...
| `cash_variance_note_threshold_cents` | bigint | null: false; default `100`; check `>= 0` | Organization configurable default; absolute session/safe variance at or above this requires an additional free-text note. Any nonzero variance still requires a managed reason code ([phase11-schema.md](../phase11-cash-accountability/phase11-schema.md) §8) |
| `cash_variance_approval_threshold_cents` | bigint | null: false; default `1000`; check `>= 0` | Organization configurable default; absolute variance at or above this is material ([phase11-schema.md](../phase11-cash-accountability/phase11-schema.md) §8) |
| `cash_paid_out_approval_threshold_cents` | bigint | null: false; default `5000`; check `>= 0` | Organization configurable default; paid-out amount at or above this uses `cash.approve_paid_out` unless the performer is `direct` ([phase11-schema.md](../phase11-cash-accountability/phase11-schema.md) §8) |
