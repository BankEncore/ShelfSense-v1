...
| Summary sidebar | `workspaces/_totals.html.erb` (`section#pos_totals`), `_tenders.html.erb` → `.pos-summary-rail` |
| Keypad bar | `workspaces/_actions.html.erb` → `.pos-actions*` (Close Session: hidden F10 proxy only) |
| F10 menu panel | `shell/_menu.html.erb` → `#register-menu`, `.pos-register-menu*` |
| Dialog overlay panel | `workspaces/_overlays.html.erb` → `overlays/*` workflow partials |

Hidden command forms: `workspaces/_forms.html.erb` → `forms/*` workflow partials.
