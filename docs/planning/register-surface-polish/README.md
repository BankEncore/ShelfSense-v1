...
Status: **Accepted / implemented** on `main` (PR #131). Follow-on hierarchy polish on `register-workspace-hierarchy` (open for review; merge after manual sign-off).
...
- Coordinated Turbo Stream multi-replace and Stimulus controller splits (frozen `#pos_workspace` boundary)
- ESC/POS or raw printer drivers
- Full a11y matrix **conforming** cashier timed study (may remain open as for UDS-3)

## Hierarchy follow-on policy

- **Close Session:** not shown on the visible keypad; F10 Register Menu only, via hidden `data-register-shell-proxy="close-session"` while `close_session_available` (availability rules unchanged).
- Sale-entry punctuation shortcut hint removed from the command card (keypad labels remain authoritative).
