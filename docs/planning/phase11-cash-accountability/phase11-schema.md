...
| `approved_by_id` | uuid, nullable | When policy required a second user |
...
Each successful non-reverse operation has exactly one source row. Inverse `has_one` from the operation.
...
One successful initialize per store safe (unique `cash_location_id` where not reversed—or application invariant plus unique constraint on non-reversed). Count, notes, manager approval, `cash_operation_id`. Typed as initialization, not paid-in.

### 4.5 `cash_deposits`

Store, business date, deposit number, optional bag/reference, total, prepared_by, approved_by, timestamps, `cash_operation_id` unique, `reversal_of_id` on a reversing deposit/operation. MVP: one business date per deposit. Bank-posted amount is **not** a column.
...
Optional `cash_count_denomination_lines` (`quantity`, `denomination_cents`). When any line exists, lines must sum to `total_cents`. Session open/close may omit lines. Safe/deposit policy: [phase11-safe-and-reporting.md](phase11-safe-and-reporting.md).
...
Organization (not per-store) integer-cent thresholds, names TBD at implementation:

- variance magnitude above which reason/note is required
- variance magnitude above which a distinct approver is required
- paid-out amount above which a distinct approver is required

Credits/debits of **session** cash via paid-in/out use these, not stored-value adjust thresholds. Preferred retained-till cash is **out of MVP** (available cash = expected session cash).
