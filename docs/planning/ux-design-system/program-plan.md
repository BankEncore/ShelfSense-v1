...
| **Phase 7.1.3** | UDS-1/UDS-2 ops tokens; `.ops-queue`, `.ops-shortcuts*`, `.location-*`, draft-PO workspace selectors named in PR; shared Stimulus only when extracted for Location/Draft PO + Receiving parity | `app/views/ops/locations/**`; `app/views/ops/draft_pos/**`; `app/views/ops/shared/_error_summary.html.erb`, `_inline_row_error.html.erb` when touched; optional shared ops Stimulus named in PR | `app/views/ops/locations/show.html.erb`; `app/views/ops/draft_pos/{index,show}.html.erb` and related partials |
| **UDS-4.0** | None on `main` (disposable prototype branch) | Navigation view-model/helper prototype only; no production layout merge | Prototype routes/views named in PR description |
| **UDS-4.1** | `.app-nav`, `.app-nav-group`, `.app-nav-group__heading`, `.app-nav-group__list`, and related grouped-nav selectors named individually in PR | `app/helpers/application_helper.rb` or dedicated nav helper; `app/views/layouts/application.html.erb` (grouped primary nav only—no domain screens) | Navigation helper/view tests; layout integration tests |
| **UDS-4.2** | UDS-1 primitives on allowlisted non–Phase-7.1 matrix rows only | Shared partials when migrating allowlisted admin families | Matrix paths explicitly listed in PR (exclude `admin/orders/**`, `admin/purchase_orders/**`, `admin/purchase_receipts/**`, `admin/purchasing/**`, `ops/locations/**`, `ops/draft_pos/**`) |

Ownership split: [phase7.1-uds-coordination.md](../phase7.1-purchasing-polish/phase7.1-uds-coordination.md) (Accepted August 2026). Phase 7.1 owns purchasing hub, purchasing admin indexes, and Location/Draft PO ops parity. UDS-4 owns grouped nav chrome and non-purchasing adoption. A needed change to one of these is a separate behavior slice, not incidental UDS work.
...
| Phase 7.1.1 | UDS-1 indirect-impact list | Hub sections per permission profile; flat nav link to hub; no grouped nav |
...
| Phase 7.1.3 | UDS-2 ops list (`purchasing_ops_workspace_test.rb`, `location_queue_buttons_test.rb`, receiving tests) | Location and Draft PO keyboard/dirty/Turbo parity with Receiving reference |
...
- **UDS-4.0** — Disposable navigation prototype; [navigation prototype gate](navigation-proposal.md#required-prototype-gate) evidence (profiles A and B).
- **UDS-4.1** — Ship grouped admin nav in `application.html.erb`; regroup existing links only; include Phase 7.1 hub link when present.
