...
`register_number` is a positive integer unique within the store. Leading zeroes are presentation only. It becomes immutable once the Register has issued a completed transaction (`receipt_sequence > 0`). Next number assignment is `max(register_number) + 1` and does not recycle gaps.
