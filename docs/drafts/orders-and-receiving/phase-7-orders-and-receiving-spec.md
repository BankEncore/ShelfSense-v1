...
- [Phase 3 inventory foundation](../../planning/phase3-inventory-foundation/phase-3-inventory-foundation.md)
- [Inventory posting service contract](../../planning/phase3-inventory-foundation/inventory-posting-contract.md)
- the implemented Phase 4–6 POS contracts under [phase4-6-point-of-sale](../../planning/phase4-6-point-of-sale/), including the [Register workspace](../../planning/phase4-6-point-of-sale/phase5-cash-register/register-workspace.md)
...
| Availability enforcement | Ordinary POS sales hard-stop against `available` quantity and against allocated Used units. Customer pickup links an `available` allocation to a POS line; fulfillment occurs only on successful transaction completion. Manually marking a request completed without POS is out of Phase 7. |
| Operator chrome | Location queue, draft PO, and receiving are Register-class ops workspaces (dedicated layout, Importmap + Turbo + Stimulus). The existing POS Register workspace stays independent—do not fold POS into a shared ops shell in Phase 7. Customer pickup extends the Register workspace. Admin chrome remains for configuration and history. |
...
| `store_id` | Required destination store. |
| `supplier_id` | Required supplier. |
| `number` | Required store-scoped receiving-document number, assigned at creation or posting according to the implementation plan; never reused after posting. |
...
Availability is an application contract, not a UI hint. Named inventory/POS callers that sell or allocate stock—at minimum request routing, locate confirmation, ordinary merchandise add/`Inventory::PostSale`, and customer pickup—must evaluate the same availability rules under their concurrency boundaries. An ordinary sale that would leave `available` negative, or that targets an allocated Used `inventory_unit`, must fail.
...
Concurrent pickup attempts must lock/validate the allocation so it cannot be fulfilled twice. An ordinary POS line must reject an allocated Used unit and must not consume the last Standard available quantity reserved for another customer.
...
Add these named services to the inventory posting contract:

| Service | Purpose | Required source |
|---|---|---|
| `Inventory::PostReceipt` | Add purchase-received quantity and merchandise value using moving weighted average | `PurchaseReceiptLine` |
| `Inventory::ReverseReceiptLine` | Exact inverse of eligible posted receipt-line physical and valuation effects | `PurchaseReceiptLineCorrection` |
| `Inventory::CorrectReceiptLineCost` | Valuation-only delta for a cost error with correct quantity | `PurchaseReceiptLineCorrection` |
...
Implementation planning must publish one deterministic lock order covering:

1. purchase receipt or PO aggregate root;
2. PO lines and line processing state;
3. orders and customer requests/allocations;
4. inventory balances; and
5. any POS rows required by pickup completion.

The exact order must be reconciled with the already implemented POS and Inventory service lock orders. No caller may pre-lock an inventory row in an order that conflicts with the named posting kernel.
...
Authority for the Register pattern: [register-workspace.md](../../planning/phase4-6-point-of-sale/phase5-cash-register/register-workspace.md). Purchasing ops workspaces follow the same governing UX principle: where are the operator’s eyes, where is keyboard focus, and what will the next keystroke do?
...
- suppliers;
- optional supplier-variant sources;
- organization preference and store override;
- eligibility/default resolver;
- permissions, audit, tests, and admin UI.

Deliverable: buyer can configure a supplier/source or select a supplier without a source.

### Slice 7.2 — Customers and requests

- minimal customers;
- quantity-one customer requests;
- Standard/Used request eligibility and automatic initial routing;
- location ops workspace;
- Standard physical confirmation and exact Used-unit confirmation;
- active allocation/availability integration (hard-stop contract for later POS use);
- not-located Standard conversion/cancellation and Used cancellation;
- request permissions, audit, keyboard UX, and UI.

Deliverable: user can create a request, route it correctly from availability, and locate/reserve apparent in-stock Standard or Used merchandise in the location ops workspace.

### Slice 7.3 — Orders and draft POs

- orders and replacement lineage;
- automatic supplier/store draft PO grouping;
- dedicated PO lines (one order → one line);
- source defaults and editable expected economics;
- stock creation and Standard special-order creation/conversion paths;
- draft PO ops workspace.

Deliverable: stock order or Standard special order becomes a valid draft PO line in the draft PO ops workspace; Used variants are rejected at every purchasing boundary.

### Slice 7.4 — PO generation, send, cancellation, and re-sourcing

- store-scoped numbering;
- document generation/revision;
- send lock point;
- optional line-state acknowledgment;
- cancellation facts;
- linked replacement order workflow;
- automatic PO close derivation.

Deliverable: buyer can send an immutable PO and manage unresolved quantity without rewriting it.

### Slice 7.5 — Physical receiving and inventory posting

- purchase receipts and lines;
- multi-PO same-supplier/store selection;
- matched versus unplanned quantity;
- actual merchandise cost;
- ancillary charge capture;
- `Inventory::PostReceipt`;
- receiving ops workspace;
- idempotency, audit, outbox, and projection tests.

Deliverable: one physical delivery posts auditable inventory and valuation and updates PO open quantities from the receiving ops workspace.

### Slice 7.6 — Customer allocation and POS pickup

- receipt-created Standard special-order allocation;
- availability hard-stop integrated into ordinary Register sale paths;
- customer cancellation release;
- Register-workspace request lookup and Standard/Used pickup;
- successful completion fulfillment;
- no admin “mark completed” path.

Deliverable: located or received request merchandise is withheld from general availability and sold through the existing POS Register workspace to complete the request.

### Slice 7.7 — Corrections and closeout

- receipt-line and whole-receipt reversal convenience;
- cost-only valuation correction;
- downstream-conflict compensating workflow;
- final lifecycle, authorization, concurrency, and reconciliation coverage;
- end-to-end operator tests.

Deliverable: errors are correctable without editing posted facts, and POs/requests close consistently.
...
- PO numbers are unique and sequential within store and are never reused.
...
- Ordinary POS sale cannot consume an allocated Used unit or the Standard quantity reserved for another request.
...
- Location, draft PO, and receiving use dedicated Register-class ops workspaces; POS Register remains a separate workspace and is not folded into a shared ops shell in Phase 7.
...
- ordinary POS sale rejection for reserved Standard availability and allocated Used units;
